...
async function showAdmin(){
  ...
  try{
    ...
    document.getElementById("rzAdminPanel").innerHTML='<section class="rz-enterprise-stats rz-card">'+stat("Users",s.totalUsers)+stat("Active",s.activeUsers)+stat("Suspended",s.suspendedUsers)+stat("Verified",s.verifiedUsers)+stat("Posts",s.totalPosts)+stat("Communities",s.totalCommunities)+stat("Reports",s.openReports)+stat("Tickets",s.openTickets)+stat("Payments",s.successfulPayments)+stat("Pending",s.pendingPayments)+'</section><section class="rz-card"><div class="rz-section-title"><h3>Support queue</h3>'+btn("Refresh","refresh-admin")+'</div><div class="rz-feed">'+(d.latestTickets||[]).map(adminTicket).join("")+'</div></section><section class="rz-card"><div class="rz-section-title"><h3>Moderation queue</h3></div><div class="rz-feed">'+(d.latestReports||[]).map(adminReport).join("")+'</div></section><section class="rz-card"><h3>Payment ledger</h3><div class="rz-feed">'+(d.latestTransactions||[]).map(function(t){return '<div class="rz-card"><strong>'+esc(t.reference)+'</strong><div>'+esc(t.amountNaira)+' '+esc(t.currency)+' · '+esc(t.status)+'</div></div>';}).join("")+'</div></section>';
    bindAdmin();
