...
async function earnings(){
  try{
    ...
    m.querySelector("#rzBusinessExport").onclick=async function(){try{var rr=await fetch(API+"/api/v2/business/export",{credentials:"include"});if(!rr.ok)throw new Error("Export failed.");var blob=await rr.blob(),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="rizora-business-ledger.csv";document.body.appendChild(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(url);},1000);}catch(e){toast(e.message);}};
